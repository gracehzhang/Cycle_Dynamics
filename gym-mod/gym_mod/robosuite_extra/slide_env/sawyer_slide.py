...
import numpy as np
import gym
from environments.robosuite_extra.env_base import SawyerEnv
from robosuite.models.arenas import EmptyArena
from environments.robosuite_extra.models.generated_objects import FullyFrictionalBoxObject
from environments.robosuite_extra.slide_env.slide_task import SlideTask
from environments.robosuite_extra.utils import transform_utils as T
from environments.robosuite_extra.controllers import SawyerEEFVelocityController
import copy
...
class SawyerSlide(SawyerEnv):
    ...
    def __init__(
        ...
        self.action_space = gym.spaces.Box(high=np.ones(2), low=-np.ones(2))
    ...
    def observation_space(self):
        ...
        for k, v in observation.items():
            observation_space[k] = gym.spaces.Box(low=-1.0, high=1, shape=v.shape)
    ...
    def _set_default_dynamics_parameters(self, use_pid):
        ...
        self.default_dynamics_parameters['obj_density'] = np.asarray(500)
        self.default_dynamics_parameters['obj_size'] = np.array([0.0555 / 2, 0.0555 / 2, 0.03 / 2])
        self.default_dynamics_parameters['obj_sliding_friction'] = np.array(2.0) #np.asarray(0.3)
        self.default_dynamics_parameters['obj_torsional_friction'] = np.array(0.3) #np.asarray(0.01)
    ...
    def _reset_internal(self):
        ...
        self._start_goal_pos = goal_pos = self.sim.data.site_xpos[self.goal_site_id]
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            ...
            # goal_distance_reward = 0.1 + 0.9 * np.exp(-(10 * dist)**2)
            goal_distance_reward = -1.0 + np.exp(-(10 * dist)**2)
            # goal_distance_reward = - dist
            reward += goal_distance_reward
            
            # goal moved distance reward
            # goal_dist = np.linalg.norm(goal_pos - self._start_goal_pos)
            # goal_moved_distance_reward = 0.5 * (-1.0 + np.exp(-(5 * goal_dist)**2))
            # reward += goal_moved_distance_reward
    ...
    def _check_success(self):
        ...
        if (dist <= 0.023 and goal_dist <= 0.023):
            return True
    ...
    def _pre_action(self, action):
        ...
        self.joint_random_forces = self.dynamics_parameters['joint_forces'] * np.zeros(2) #self.np_random.uniform(-1, 1, 2)
        ...
        self.object_random_forces = self.dynamics_parameters['obj_forces'] * np.zeros(6) #self.np_random.uniform(-1, 1, 6)

...
class SawyerPushEnv(SawyerEnv):
    ...
    def compute_reward(self, action):
        reward_type = "dense" #self._kwargs["reward_type"]
        info = {}
        reward = 0
        ...
        if reward_type == "dense":
            ...
            reward_push = -(cylinder_to_target > 0.06)
            reward += reward_reach
            reward += reward_push
    ...
    def _get_obs(self):
        ...
        di = np.concatenate([k for (v,k) in di.items()])
        di = np.concatenate([super_di, di])
        return di
...
class SawyerPushBackground(SawyerPushEnv):
    def __init__(self, **kwargs):
        SawyerEnv.__init__(self, "sawyer_push_background.xml", **kwargs)
        self._get_reference()
        self.sim.model.geom_friction[self.cylinder_geom_id][0] = kwargs["puck_friction"]
        self.sim.model.body_mass[self.cylinder_body_id] = kwargs["puck_mass"]
        self.sim.set_constants()

class SawyerPushShiftView(SawyerPushEnv):
    def __init__(self, **kwargs):
        kwargs["camera_name"] = "shiftview2"
        SawyerEnv.__init__(self, "sawyer_push_color.xml", **kwargs)
        self._get_reference()
        self.sim.model.geom_friction[self.cylinder_geom_id][0] = kwargs["puck_friction"]
        self.sim.model.body_mass[self.cylinder_body_id] = kwargs["puck_mass"]
...
class SawyerPushShiftViewBackground(SawyerPushEnv):
    def __init__(self, **kwargs):
        kwargs["camera_name"] = "shiftview2"
        SawyerEnv.__init__(self, "sawyer_push_color.xml", background="Interior", **kwargs)
        self._get_reference()
...
class SawyerPushColorShiftView(SawyerPushEnv):
    def __init__(self, **kwargs):
        kwargs["camera_name"] = "shiftview"
        SawyerEnv.__init__(self, "sawyer_push_color.xml", **kwargs)
        self._get_reference()
        self.sim.model.geom_friction[self.cylinder_geom_id][0] = kwargs["puck_friction"]
        self.sim.model.body_mass[self.cylinder_body_id] = kwargs["puck_mass"]
...
class SawyerPushRobotTopView(SawyerPushEnv):
    def __init__(self, **kwargs):
        kwargs["camera_name"] = "tabletopview"
        SawyerEnv.__init__(self, "sawyer_push_robot.xml", **kwargs)
        self._get_reference()
        self.sim.model.geom_friction[self.cylinder_geom_id][0] = kwargs["puck_friction"]
        self.sim.model.body_mass[self.cylinder_body_id] = kwargs["puck_mass"]
        self.sim.set_constants()
    ...
    def _reset(self):
        ...
        if self._kwargs["task_level"] == "easy":
            init_target_qpos = np.array([0.2, 0.0])
            init_target_qpos += self.np_random.randn(init_target_qpos.shape[0]) * 0.02
        else:
            init_target_qpos = self.np_random.uniform(low=-3, high=3, size=2)
        self.goal = init_target_qpos
